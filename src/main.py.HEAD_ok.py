...
def loop_prompt_output(input: str, model: Small_LLM_Model,
    ...
    for idx, arg in enumerate(args_fn):
        ...
        if arg_type == "number":
            ...
            # A "regex" argument is resolved by the program itself (concept
            # -> pattern) instead of letting the model guess, which also
            # avoids any get_logits call for this value.
            if arg == "regex":
...
def inferir_patron_regex(user_prompt: str) -> str | None:
    """Infer a regex pattern from a concept word found in the user prompt.

    Generic (concept -> pattern), not tied to any specific function. It is
    only used when the argument name is ``regex`` and the prompt mentions one
    of a small set of known concepts (vowels / numbers / a literal quoted
    word). Returns ``None`` when no concept is recognised so the caller can
    fall back to the normal model-driven choice.

    Implemented without ``re`` (only ``split``/``strip``) to respect the
    "only json and numpy" constraint of the subject.

    Args:
        user_prompt: The raw user request text.

    Returns:
        The regex pattern string to force, or ``None`` if no concept matched.
    """
    texto = user_prompt.strip()
    lower = texto.lower()

    concept_to_regex = [
        (("vowel", "vocal", "aeiou"), r"[aeiouAEIOU]"),
        (("number", "digit", "num"), r"\d+"),
    ]
    for conceptos, patron in concept_to_regex:
        if any(concepto in lower for concepto in conceptos):
            return patron

    # A literal quoted word (e.g. 'cat') becomes an escaped literal pattern.
    for palabra in texto.split():
        limpia = palabra.strip(".,;:!?\"'")
        if limpia and ((palabra.startswith("'") and palabra.endswith("'"))
                       or (palabra.startswith('"') and palabra.endswith('"'))):
            return limpia
    return None
...
def codificar_valor_string(model: Small_LLM_Model, vocab: dict,
                           texto: str) -> list[int]:
    """Encode an ALREADY-DECIDED string value (not chosen by the model).

    Used for values that the program resolves itself (e.g. a forced regex
    pattern). Backslashes are doubled before encoding so that a pattern like
    ``\\d+`` survives ``json.loads`` as the two characters backslash + d,
    instead of being read as an invalid JSON escape.

    Args:
        model: Instance of the Qwen3-0.6B model (for tokenizing).
        vocab: Mapping of text fragments to their token IDs.
        texto: The text value to force (without surrounding quotes).

    Returns:
        List of token IDs including the opening and closing quote.
    """
